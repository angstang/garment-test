...
for each skill:
  ...
  while round < MAX_ROUNDS (默认3):
    ...
    if 新总分 > 旧总分:
      ...
      # 二次确认：改进失败时不直接回滚
      展示分数下降的原因分析
      问用户: "放弃本次改进？(Y=rollback / N=保留为experimental)"
      if 用户选Y:
        status = "revert"
        git revert HEAD（创建新commit回滚）
        记录失败尝试到 results.tsv
        break
      else:
        status = "experimental"
...
```
格式: "[行 X-Y] {改动摘要} | 原因: Rubric#{维度} | 预期: +{N}分"

示例:
  [42-48] 补充"SKILL.md格式破损"的处理逻辑
    原因: Rubric#3(边界条件)
    预期: +1.5分
```

**检查清单:**
- [ ] 修改的行范围明确（避免"整个段落"的模糊描述）
- [ ] 改动内容不与上轮改动冲突（git diff检查）
- [ ] 只针对一个维度（不允许"顺便改一下其他"）
- [ ] 预期分数提升量合理（0.5-2分范围）
...
| 分数 | 判断标准 |
|------|---------|
| **9-10** | 🟢 优秀：带skill输出明显好于baseline（>60%质量提升）+ 无任何负面影响 |
| **7-8** | 🟡 良好：带skill输出好于baseline（30-60%质量提升），可能有轻微的冗余 |
| **5-6** | 🟠 中等：与baseline接近或略优，不足以说明skill的价值 |
| **3-4** | 🔴 较差：带skill反而比baseline差，或输出格式不符预期 |
| **1-2** | 🔴 极差：skill完全失效或导致错误 |

**跑测试时的注意事项:**
- 测试prompt应是真实的使用场景，不是边缘case
- 对比 with_skill vs baseline 的输出，不只看快慢，看质量
- 如果多个测试prompt分数不一致，取平均值
- 记录具体的输出对比示例到 results.tsv 的"评估详情"列
...
本优化循环必须处理以下边界条件和异常，确保生产可靠性：

### 初始化阶段 (Phase 0) 的容错

| 异常情况 | 处理方案 |
|---------|---------|
| **skills目录为空或找不到SKILL.md** | 提示"未找到任何SKILL.md文件"，建议用户创建第一个skill，退出 |
| **git不可用或不在repo中** | 提示"需要在git repo中执行"，提供初始化命令，退出 |
| **results.tsv损坏** | 用git恢复到上一个已知良好版本：`git show HEAD:auto-optimize-results.tsv > results.tsv` |
| **无法创建优化分支** | 如果分支已存在，提示用户清理或切换分支，不要强制删除 |

### 评估阶段 (Phase 0.5-1) 的容错

| 异常情况 | 处理方案 |
|---------|---------|
| **某个SKILL.md格式破损** (YAML解析失败) | 使用`git show HEAD~1:SKILL.md`恢复上一版本，报告错误行号，继续处理其他skills |
| **test-prompts.json不存在** | 创建骨架文件，提示用户补充测试prompt，不能跳过此步 |
| **测试prompt无法执行** (子agent超时) | 降级为干跑验证(dry_run模式)，标注到results.tsv，继续但标记为不完整评估 |
| **评分被用户拒绝** | 保存评分数据到results.tsv，status=rejected，询问用户是否修改rubric重新评分 |

### 优化循环 (Phase 2) 的容错

| 异常情况 | 处理方案 |
|---------|---------|
| **git conflict在commit时** | 中止本轮改进，`git merge --abort`，标记为status=conflict，询问用户如何处理 |
| **改进后SKILL.md超过200%原始大小** | 拒绝提交，提示"可能改进过度"，建议用户审查改动 |
| **评分下降但改进内容合理** | 不自动回滚，展示原因分析，询问用户是否为experimental版本 |
| **连续3轮都无改进(plateau)** | 停止该skill的优化，建议"探索性重写"(Phase 2.5)，询问用户是否同意 |

### 中断和恢复 (用户 Ctrl+C)

| 场景 | 处理方案 |
|------|---------|
| **优化中途中断** | 如果正在编辑SKILL.md，立即提交WIP commit: `git commit -m "WIP: optimization interrupted"` |
| | 保存当前状态，提示用户可用 `git branch` 查看工作分支 |
| | 恢复工作: `git checkout auto-optimize/<branch>` + `git rebase -i` 清理WIP commit |
| **评估中途中断** | 保存已评估的skills结果到results.tsv，其余skills标记为pending |

### 重试策略

- **第一次失败:** 标记到results.tsv，显示失败原因，询问用户"重试？(Y/N)"
- **连续失败3次:** 放弃该skill，进入下一个
- **若所有skills都失败:** 建议用户检查test-prompts.json或rubric定义是否合理
